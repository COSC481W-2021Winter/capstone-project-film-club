...
class HomePageProfileTestCase(LiveServerTestCase):
    ...
    def test_profile(self):
        ...
        movie2 = Movie(title = 'Test Movie 2', description = 'literally not a real movie 2', api_id = 2, poster_path = '/image2.png')

        movie1.save()
        movie2.save()

        review1 = Review(user = self.user, movie = movie1, title = 'Test Review', text = 'literally not a real review', score = 1)
        review2 = Review(user = self.user, movie = movie2, title = 'Test Review 2', text = 'literally not a real review 2', score = 2)
